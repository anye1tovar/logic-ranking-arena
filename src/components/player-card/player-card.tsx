...
export const PlayerCard = ({
	...
	const startMoveEffect = useCallback(() => {
		setMoveDelta(effectState?.moveDelta ?? 0);
	}, [effectState?.moveDelta]);

	useTransientFlag(
		effectState?.newPlayerTrigger,
		"newPlayer",
		setActiveFlags,
		VIEWER_EFFECT_TIMING.card.newPlayerMs,
	);
	useTransientFlag(
		effectState?.winTrigger,
		"win",
		setActiveFlags,
		VIEWER_EFFECT_TIMING.card.winMs,
	);
	useTransientFlag(
		effectState?.classifiedTrigger,
		"classified",
		setActiveFlags,
		VIEWER_EFFECT_TIMING.card.classifiedMs,
	);
	useTransientFlag(
		effectState?.eliminatedTrigger,
		"eliminated",
		setActiveFlags,
		VIEWER_EFFECT_TIMING.card.eliminatedMs,
	);
	useTransientFlag(
		effectState?.championTrigger,
		"champion",
		setActiveFlags,
		VIEWER_EFFECT_TIMING.card.championMs,
	);
	useTransientFlag(
		effectState?.moveTrigger,
		"move",
		setActiveFlags,
		VIEWER_EFFECT_TIMING.card.moveMs,
		startMoveEffect,
	);
	...
	return (
		<motion.article
			...
			className={clsx(
				styles.card,
				styles[`status${capitalize(player.status)}`],
				{
		...
		>
			<div className={styles.overlayEffects}>
				{showScoreImpact && latestFlash ? (
					<motion.div
						...
						animate={{
							opacity: [0, 0.95, 0],
							scale: [0.6, latestFlash.value >= 3 ? 1.28 : 1.18, 1.58],
						}}
						transition={{
							duration:
								latestFlash.value >= 3
									? VIEWER_EFFECT_TIMING.motion.scoreRippleStrongSeconds
									: VIEWER_EFFECT_TIMING.motion.scoreRippleSeconds,
							ease: "easeOut",
						}}
					/>
				) : null}
				<AnimatePresence>
					{activeFlags.classified ? (
						<motion.div
							...
							exit={{ opacity: 0 }}
							transition={{
								duration: VIEWER_EFFECT_TIMING.motion.classifiedSweepSeconds,
								ease: "easeInOut",
							}}
						/>
					) : null}
				...
				<AnimatePresence>
					{activeFlags.eliminated ? (
						<motion.div
							...
							exit={{ opacity: 0 }}
							transition={{
								duration: VIEWER_EFFECT_TIMING.motion.eliminatedSeconds,
								ease: "easeOut",
							}}
						>
							<img
								className={styles.eliminationAnimation}
								src={eliminatedNoxGif}
								alt=""
								draggable={false}
							/>
							<span className={styles.eliminationText}>ELIMINATED</span>
						</motion.div>
					) : null}
				</AnimatePresence>
				<AnimatePresence>
					{activeFlags.newPlayer ? (
						<motion.div
							...
							exit={{ opacity: 0 }}
							transition={{
								duration: VIEWER_EFFECT_TIMING.motion.newPlayerSeconds,
								ease: "easeOut",
							}}
						>
							NEW ENTRY
				...
				<AnimatePresence>
					{activeFlags.win ? (
						<motion.div
							...
							exit={{ opacity: 0 }}
							transition={{
								duration: VIEWER_EFFECT_TIMING.motion.winSeconds,
								ease: "easeOut",
							}}
						>
				...
				<AnimatePresence>
					{activeFlags.move && moveLabel ? (
						<motion.div
							...
							animate={{
								opacity: [0, 1, 0],
								x: [moveDelta < 0 ? -20 : 20, 0, 0],
								scale: [0.86, 1, 0.96],
							}}
							exit={{ opacity: 0 }}
							transition={{
								duration: VIEWER_EFFECT_TIMING.motion.moveSeconds,
								ease: "easeOut",
							}}
						>
			...
			<div className={styles.body}>
				...
				<Typography className={styles.subline}>
					{player.wins} victorias • actualizado{" "}
					{new Date(player.updatedAt).toLocaleTimeString("es-CO", {
						hour: "2-digit",
						minute: "2-digit",
					})}
				</Typography>
				{admin ? (
					<div className={styles.controls}>
						...
						<Button
							...
							onClick={() => onAddPoint?.(player.id, 3)}
						>
							+3
						</Button>
						<Button
						...
						<Button
							...
							variant="outlined"
							color="secondary"
							onClick={() => onSetStatus?.(player.id, "classified")}
						>
							Classify
						</Button>
						<Button
							...
							variant="outlined"
							color="error"
							onClick={() => onSetStatus?.(player.id, "eliminated")}
						>
							Eliminate
						</Button>
						<Button
							...
							onClick={() => onSetStatus?.(player.id, "champion")}
						>
							Champion
						</Button>
						<Button
					...
					{player.scoreFlashes.map((flash, index) => (
						<motion.div
							...
							animate={{
								...
								scale: viewerMode
									? [0.75, flash.value >= 3 ? 1.34 : 1.22, 1]
									: 1,
							}}
							exit={{
								opacity: 0,
								y: -(viewerMode ? 72 : 42) - index * 14,
								scale: 0.82,
							}}
							transition={{
								duration: viewerMode
									? VIEWER_EFFECT_TIMING.motion.floatingSeconds
									: 1.1,
								ease: "easeOut",
							}}
						>
